...
def check_brackets(row: str) -> bool:
    stack = []
    ...
    for el in row:
        ...
        if el in closing_brackets:
            bracket_set = closing_brackets.index(el)
            opening_bracket_of_set = open_brackets[bracket_set]

            if len(stack) == 0 or opening_bracket_of_set != stack[-1]:
                return False
